...
int doffire(unsigned tmax=1000000, int doSQL=1, unsigned short fMatch=200, unsigned short nRand=1, unsigned short nClust=0, int kmax=4) {
...
*/
	...
	for (unsigned int i=0; i<=tmax; i++) {
		...
		//
		bool keepWalkin=0, isClust=0, doWalk=0;
		int iwalk=0, iwalkmax=0;

		if (rcPlant.nextDouble() <= float(nClust)/float((nClust+nRand))) {
			doWalk=1;
			keepWalkin=1;
			};
		// do a random walk to look for a cluster
		//
		iwalkmax = 5000 + 25000/(i+1);
		while (iwalk<=iwalkmax and keepWalkin==1 ) {
			// calc grid position and check for "sticking" conditions, then walk at the end of the loop.
			gridPos = (xmax+2)*(thisY+1)+1+thisX;
			if (hasNeighbor(grids, gridPos)>=1 and hasNeighbor(grids, gridPos)<=kmax) {
				keepWalkin=0;	// we might set dendritic conditions like hasNeighbors()>0 and <=2...
				isClust=1;
				};
			//
			// do the walk. if we already found neighbors, this walk will evaporate.
			sWalkDir = walkDir.nextInt(4);
			switch (sWalkDir) {
				case 0:
					//xWalk = 1;
					thisX++;
			...
			if (keepWalkin==0 or thisX>int(xmax) or thisX<1 or thisY>int(ymax) or thisY<1) keepWalkin=0;
			iwalk++;
			};
		//
		if (*(grids + gridPos) < rockAge and isClust==1 or doWalk==0) *(grids + gridPos) = 1; //thisClust;
		//
// debug:
//		if (i%10000==0) {
