...
int doffire(unsigned tmax=1000000, unsigned short fmatch=200, unsigned int randPlant=1, unsigned int clustPlant=0, unsigned short burnAge=1, unsigned short burnPropAge=1, int rhoRocks=0, int doSQL=1, float pImmune=0, int nnnAge=0) {
	...
	if (nnnAge==0) nnnAge=99999;	// trees age>nnnAge will spread to next nearest neighbors. use "0" pram for no nnn connectivity; nothing should ever get this old.
	int aveRho = 0, aveVol=0;
	//
	//printf("comp cycle match freq: %d, %d\n", fmatch, fmatch/(randPlant + clustPlant));
	//
	std::string simName("ForestFire5b-nnn");
...
//	int thisStatus;
	...
	for (unsigned int i=0; i<(xmax+2)*(ymax+2); i++) {
		...
		if (rand()%(xmax)==1 and i%(xmax+2)!=0 and i%(xmax+2)!=(xmax+1) and i>(xmax+2) and i<((xmax+2)*(ymax+1)) ) grids[i]++;
		// also plant some unburnable places. use gridVal=12345 (see const); we'll have to be sure nothing else ever gets that old.
		if (rand()%100 < rhoRocks) grids[i]=rockAge; // we can plant rocks on the boundaries; we probably should plant rocks on the boundaries.
		//yodapause();
		};
...
//	unsigned int fVols[15*xmax*ymax];		// estimating max age <15
	...
	if (doSQL==1 or doSQL==5 or doSQL==25) {	
		...
		myquery << "insert into ForestFire.SimPrams (`SimName`, `xmax`, `ymax`, `sparkInterval`, `burnAge`, `tmax`, `nRand`, `nClust`, `kmax`, `pImmune`, `Comment`) values (%0q, %1q, %2q, %3q, %4q, %5q, %6q, %7q, %8q, %9q, %10q )";
		myquery.parse();
		...
		//myquery.execute(simName.c_str(), simName.c_str(), xmax, ymax, fmatch, burnAge, tmax, randPlant, clustPlant, kmax, simPramComment.c_str());
		  myquery.execute(simName.c_str(), xmax, ymax, fmatch, 1, tmax, 1, 0, 4, pImmune, simPramComment.c_str());
		//
	...
	//			};
			if (*thisSquare<rockAge) *thisSquare = *thisSquare + 1;
			};
		// yoder: 27 feb 2008: impose random/clustering planting ratios:
		// clustered planting:
		//for (unsigned int icp=0; icp<clustPlant; icp++) {
// 8 june 2008 yoder: below is not a scale invariant method of cluster planting. see ForestFire7b
// for a more appropriate cluster filling model (not quite what we were hoping for here, but it works).
/*
		if (clustPlant > 0 and i%(yodacode::int1if0(clustPlant))==0 and 1==2) {
			...
			while (doPlant==0 and icount<(xmax+2)*(ymax+2) ) {
				...
				if (grids[startPos] > 0 and grids[startPos]<rockAge) treesSoFar++;
				if (treesSoFar > (xmax/3) or (grids[startPos]==0 and (
					...
					//grids[startPos]++;
					if (grids[startPos]<rockAge) grids[startPos] = grids[startPos]+1;
//					nTreesPlanted++;
					doPlant=1;
...
*/
							if (cStat >= burnAge and cStat < rockAge 
								and ( (lStat<=-burnPropAge or rStat<=-burnPropAge or uStat<=-burnPropAge or dStat<=-burnPropAge)
									or (nnnAge>=1 and (ulStat<=-burnPropAge or urStat<=-burnPropAge or llStat<=-burnPropAge or lrStat<=-burnPropAge)) )
								) {
						...
						//fireVol = fireVol + dFireVol;
						// small fires might go out (we might put them out)
						//printf("immune factor: %f\n", pImmune/float(1+(nBurning/10)) );
						//yodapause();
// mean-field immunity:
						if (frandImmune < (pImmune/float(1+(float(nBurning)/5))) ) {
				...
				//FireVolTotal = FireVolTotal + fireVol;
				fcounts[nBurning-1]++;
				//fVols[fireVol-1] = fVols[fireVol-1] + fireVol;
			//	plotGrid(&grids[0], xmax, ymax);
				if (doSQL==0) {
					printGrid(&grids[0], i, xmax, ymax);
					};
