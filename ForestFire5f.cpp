...
// 5d uses immunity like: pIm*exp((1-k)/k0)
// 5e uses immunity like: A*pIm^k/k0
// 5f combines 5c,5d. we will fix 1/k type immunity to something intermediate, pIm1->1 and then vary the second function
// to explore the combination of two types of immunity.
// the "weather" (random number) is the same for both immunities
//
				...
				*fireSquare=-(*fireSquare);	// the fire-square starts burning
				...
				while (dnBurning > 0 and doQuench==0) {
					...
					for (int iy = (yFireMin); iy <= (yFireMax); iy++) {
						...
						//if (frandImmune < (pImmune/(1+(float(nBurning-1)/kImmune))) ) {
						if (frandImmune < 1/(2*(1+(float(nBurning-1)/6))) or frandImmune < pImmune*exp(-kImmune*(nBurning-1)) ) {
