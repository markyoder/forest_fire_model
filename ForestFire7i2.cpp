...
//
//	VERSION 7I2:
//	make all edges trapping regions, for consistency...
//
...
int doffire(unsigned tmax=1000000, int doSQL=1, unsigned int fMatch=200, float nRand=1, float nClust=0, int kmax=4, int sheepSpeed=0, int nSheep=0) {
	// burn toplolgy: do we nave nnn burning
	// 
	// allow for no-fire simulation (aka, sheep only):
	if (fMatch==0) fMatch=tmax;
	//
	std::string simName("ForestFire7i");
	std::string simPramComment("random walk, trapping sheep on right");
	//int fMatch = 250;
	//
...
*/
	...
	//	bool sheepIsEating=0;
		...
		//		};
			...
			//if (sheepPos[insheep]<(xmax+2)) sheepPos[insheep]=sheepPos[insheep] + ymax*(xmax+2);	// if he wanders off the top...
			...
			// a simpler way to say off top/bottom:
			// let's make all boundaries trapping regions. if we step off the grid, just push them back.
			if (sheepPos[insheep]/(xmax+2)==0) sheepPos[insheep]=sheepPos[insheep] + (xmax+2);
			if (sheepPos[insheep]/(xmax+2)==(ymax+1)) sheepPos[insheep]=sheepPos[insheep]-(xmax+2);
			if (sheepPos[insheep]%(xmax+2)==0) sheepPos[insheep]=sheepPos[insheep]+1;
			if (sheepPos[insheep]%(xmax+2)==(xmax+1)) sheepPos[insheep]=sheepPos[insheep]-1;

			//if (sheepPos[insheep]/(xmax+2)==0) sheepPos[insheep]=sheepPos[insheep] + (ymax+1)*(xmax+2);
			// if (sheepPos[insheep]/(xmax+2)==(ymax+1)) sheepPos[insheep]=sheepPos[insheep]-ymax*(xmax+2);	
...
//			};
	...
	// 2 + 5, get it? insert summary to sql and return summary to screen/gnuplot.
	if (doSQL==25) { // no plots, just a summary -> SQL
		//
		for (unsigned int i=0; i<(biggestFire); i++) {
			if (*(fcounts + i) != 0) printf("%d\t%d\n", i+1, *(fcounts + i));
			myquery.reset();
