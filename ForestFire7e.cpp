...
int doffire(unsigned tmax=1000000, int doSQL=1, unsigned int fMatch=200, float nRand=1, float nClust=0, int kmax=4, int sheepSpeed=0) {
	...
	std::string simPramComment("random walk -> random down/right only");
	//int fMatch = 250;
	//
...
*/
	...
	while (fcounts[0]<tmax) {
		...
		//		};
			...
			//
			// we seemed to get a power-law result from a mistake (if (sheepPos%(xmax+1)==0) sheepPos = sheepPos+(xmax).
			// this definesa diagonal line from the top right to the bottom left of the grid.
			// so let's experiment with this mistake. protect some region from fires. start with a vertical line?
			//if (sheepPos%(xmax+2)==(xmax/2)) sheepPos=sheepPos+int(float(xmax)*.75);
			//
			if (sheepPos/(xmax+2)==0) sheepPos = sheepPos + ymax*(xmax+2);
			if (sheepPos/(xmax+2)==(ymax+1)) sheepPos = sheepPos - ymax*(xmax+2);
			if (sheepPos%(xmax+2)==0) sheepPos=sheepPos+xmax;	// off left side
			//if (sheepPos%(xmax+2)==(xmax+1)) sheepPos=sheepPos-xmax;	// off right side
			if (sheepPos%(xmax+1)==0) sheepPos=sheepPos-xmax;
			//
			if (sheepPos<0) sheepPos = -sheepPos;
			if (sheepPos>(xmax+2)*(ymax+2)) sheepPos=sheepPos-(xmax+2)*(ymax+2);
