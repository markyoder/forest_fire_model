	...
	// a 1 in fMatch chance (use any value between 0 and fMatch)
		if (rfMatch.nextInt(fMatch) == 1) {
			...
			if (*fireSquare > 0 and *fireSquare<rockAge) {
				...
				*fireSquare=-(*fireSquare);	// the fire-square starts burning
				...
				//
				// footprint:
				firefpxmin=xFire; firefpxmax=xFire; firefpymin=yFire; firefpymax=yFire; fireArea=1;
				//
				nBurning = dnBurning;
		...
		//							) {
									*centerGrid = -(*centerGrid);
									...
									dnBurning ++;
									//
									// and do the footprint:
									if (ix>int(firefpxmax)) firefpxmax=unsigned(ix);
									if (ix<int(firefpxmin)) firefpxmin=unsigned(ix);
									if (iy>int(firefpymax)) firefpymax=unsigned(iy);
									if (iy<int(firefpymin)) firefpymin=unsigned(iy);
						...
						//yodapause();
					// so, this could be optimized by only incrementing if the footprint has broadened (aka, fire has burned in x or y). note we'll
					// do this to keep track of the fire footprint; eventually, maybe work the footprint logic into the propagation logic.
				...
				//
				// and footprints:
				fireArea=(firefpxmax-firefpxmin+1)*(firefpymax-firefpymin+1);
				fireFprints[fireArea-1]++;
			//	plotGrid(&grids[0], xmax, ymax);
				if (doSQL==0) {
