...
//	int Ymax = 1000;
	...
	unsigned short fmatch = 1024;
	float pMatch = 1;
	unsigned short burnAge=1;	// a tree will burn if it comes into contact with fire and age>=burnAge
	float pBurn=.7;	// fixed probability
	float randBurn=0;	// this will be the random number
	...
	unsigned int nEvents[(ymax+2)*(xmax+2)];
	// initialize?
				...
				while (dnBurning > 0) {
					...
					for (int iy = (yFireMin); iy <= (yFireMax); iy++) {
						for (int ix = (xFireMin); ix <= (xFireMax); ix++) {
							...
							randBurn = float(rand()%1000)/1000;
							//if (*centerGrid >= 1 and (*leftGrid==-1 or *rightGrid==-1 or *upGrid==-1 or *downGrid==-1)) {
							if (*centerGrid >= 1 and randBurn<(pBurn/(1+nBurning/5)) and (*leftGrid==-1 or *rightGrid==-1 or *upGrid==-1 or *downGrid==-1)) {
