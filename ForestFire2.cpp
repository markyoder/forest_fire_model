...
int printGrid(signed int *grids, int X, int Y) {
	...
	while (i < imax) {
		...
		// yodacode::zeroTrimmed(yodacode::intToStr(*(grids+i)).c_str(), 5).c_str()
		// if (*(grids+i)>=1) printf("(%i)", *(grids+i));
		if (*(grids+i)>=1) {
			printf("(%s)", yodacode::zeroTrimmed(yodacode::intToStr(*(grids+i)).c_str(), 3).c_str());
			aveTreeAge = aveTreeAge + float(*(grids+i));
			nTrees++;
			};
		if (*(grids+i)==-1) printf("  *  ");
...
//	int Xmax = 1000;
//	int Ymax = 1000;
	std::string simName("ForestFire2");
	unsigned short fmatch = 125;
	bool doSQL = 1;
	float pMatch = 1;
	unsigned short burnAge=10000;	// a tree will burn if it comes into contact with fire and age>=burnAge
	...
	if (doSQL) {	
		...
		myquery.execute(simName.c_str(), "yodaff2", xmax, ymax, fmatch, pMatch, burnAge, tmax, wrapx1, wrapy1);
		//
		...
		myquery.parse();
		res1 = myquery.store(simName.c_str(), "yodaff2");
		...
		if (float(int(ffmatch))==ffmatch and i>xmax) {
			...
			if (*fireSquare >= burnAge) {
				...
				*fireSquare=-1;	// the fire-square starts burning
				...
				if (doSQL==0) printGrid(&grids[0], xmax, ymax);
				while (dnBurning > 0) {
					dnBurning=0;
			...
			//	plotGrid(&grids[0], xmax, ymax);
				...
				//			
				// clear grid of fire:
				// we have to age-increment trees at the begining of this block, so do this there as well.
