...
public:
    ...
    ClusterFinder(bool (*isOccupied_) (int), int xL, int yL, int* stats_, int* Grid_) {
        //isOn  = isOn_;
        ...
        int labelCnt  = int(0.5*totalSize+1); // 0.2 should work too
    ...
    void find() {
        ...
        for(unsigned i=1; i<nextLbl; i++) { // i<nextLbl, correct
            ...
            map[i] = 0;
            if(size > 0) {
            	 //printf("setting stat size(%d)\n", size);
                stats[size-1]++;
                siteCnt += size;
                //printf("finished setting stat size(%d)\n", size);
            }
        }
