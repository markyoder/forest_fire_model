...
int doffire(unsigned tmax=1000000, unsigned short fmatch=200, unsigned short burnAge=1, unsigned short burnPropAge=1, int doSQL=1) {
	std::string simName("ForestFire4");
	float pMatch = 1;
	bool wrapx1 = 0;
	//bool wrapx2 = 0;
	bool wrapy1 = 0;
...
//			};
		*thisSquare = *thisSquare + 1;
		// preferred planting (on edge of cluster):
/*
		unsigned int startPos = rand()%((xmax+2)*(ymax))+(2*(xmax+2));	// somewhere on the real grid.
		bool doPlant = 0;
		unsigned int icount=0;
		while (doPlant==0 and icount<(xmax+2)*(ymax*2)) {
			// go right or left and plan to the left/under or right/over first cluster we find. a cluster can be 1.
...
*/
	...
	if (doSQL==5 ) { // no plots, just a summary; write summary to SQL
		myquery.reset();
		printf("insert simprams.\n");
		myquery << "insert into ForestFire.SimPrams (SimName, SimSW, xmax, ymax, sparkInterval, sparkProb, burnAge, propAge, tmax, wrapX, wrapY ) values (%0q, %1q, %2q, %3q, %4q, %5q, %6q, %7q, %8q, %9q, %10q )";
		myquery.parse();
		...
		for (unsigned int i=0; i<(xmax*ymax); i++) {
			// if (fcounts[i]!=0) printf("%d\t%d\n", i+1, fcounts[i]);
			myquery.reset();
			myquery << "insert into ffcounts (simIndex, tmax, nBurned, nEvents) values (%0q, %1q, %2q, %3q, %4q)";
