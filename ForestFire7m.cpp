...
//
//	VERSION 7i:
//	seep appear to be ergotic, but we have bigger problems. it would appear that a minor mistake may have facilitated the sheep effect of 7e.
...
//
// VERSION 7m:
// yes, there are versions between j and m, but here we build on m. we did space-sheep once, but let's try it again here. nominally, we'll leave
// normal fires alone, but probably run the sim with infinite sparking frequency. sheep might be interpreted as slow burning, creeping fires.
//
...
int doffire(unsigned tmax=1000000, int doSQL=1, unsigned int fMatch=200, float nRand=1, float nClust=0, int kmax=4, int sheepSpeed=1, int nSheep=0) {
	...
	//
	std::string simName("ForestFire7m");
...
//	unsigned int biggestFire = getClustMaps1(bigClustMaps);
	...
	if (doSQL==1 or doSQL==5 or doSQL==25 or doSQL==27 or doSQL==257) {	
		//
...
*/
	//
	unsigned int ii = 0;
	unsigned long i=0;		// use "while" to increment i until we have fmax->tmax size 1 fires...
	//for (unsigned long int i=0; i<=tmax; i++) {
	while (i<=tmax) {
	//while (fcounts[0]<tmax) {
	//while (fcounts[1000]<3) {
		i++;	// note: time starts at 1...
		//if (i==(tmax-1) and ii<=10) {
		//	i=0;
		//	ii++;
		//	// printf("ii: %d", ii);
		//	};
	...
	//	bool sheepIsEating=0;
		for (int insheep=0; insheep<nSheep; insheep++) {
			...
			if (sheepRand.nextInt(sheepSpeed)==0) {
				...
				// space sheep!!!
				sheepPos[insheep] = sheepRand.nextInt(xmax) + 1 + (xmax+2)*(sheepRand.nextInt(ymax) + 1);
				while (sheepIsEating==1) {
					...
					switch (*(grids+sheepPos[insheep])) {
						case 0: case rockAge:
							...
							newSheepPoop[insheep]++;		// we have to cycle twice (or more) onto an empty/rock square before we give up eating.
							if (newSheepPoop[insheep]>1) {
								// we've finished a sheep-cluster
								if (thisSheepPoop[insheep]>0) {sheepPoops[thisSheepPoop[insheep]-1]++; };
								//printf("more sheep-poop: %d (%d)\n", thisSheepPoop[insheep], i);
								thisSheepPoop[insheep]=0;
								};
		...
		if (doSQL==6) if(i%1000000 == 0) printf("%ld million\n", i/1000000);
		//
		// PLANT A TREE:
...
//			};
	...
	// a 1 in fMatch chance (use any value between 0 and fMatch)
		if (rfMatch.nextInt(fMatch) == 1 and fMatch<tmax) {
			//yodapause();
			// throw a match.
	...
	//							) {
			...
			//	plotGrid(&grids[0], xmax, ymax);
				...
				if (doSQL==1) {
					...
					myquery.parse();
					myquery.execute(intKey, unsigned(i), xFire, yFire, nBurning, nTreesOnGrid);
					//yodapause();
					};
				if (doSQL==3) {
					printf("fire (%d) at time %ld\n",nBurning, i);
	...
	// mysqlpp::Result res2;
	if (doSQL==5 or doSQL==25 or doSQL==257) { // no plots, just a summary -> SQL
		//
		for (unsigned int i=0; i<(biggestFire); i++) {
			// if (fcounts[i]!=0) printf("%d\t%d\n", i+1, fcounts[i]);
	...
	if (doSQL==7 or doSQL==27 or doSQL==257) { // record sheep-poops...
		//
		for (unsigned int i=0; i<(biggestFire); i++) {
			// if (fcounts[i]!=0) printf("%d\t%d\n", i+1, fcounts[i]);
		//	printf ("sql bits: %d, %d, %d, %d\n", intKey, tmax, i+1, fcounts[i]);
			myquery2.reset();
			myquery2 << "insert into SheepPoop (simIndex, tmax, nEaten, nEvents) values (%0q, %1q, %2q, %3q)";
			myquery2.parse();
			myquery2.execute(intKey, tmax, i+1, sheepPoops[i]);
			//printf("%d,\t%d\n", i+1, fcounts[i]);
			};				
		}
...
//	int nnnAge = 0;
	//
	if (argv[1]) tmax=yodacode::StrToLong(argv[1]);
