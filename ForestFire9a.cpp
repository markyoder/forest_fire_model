			...
			if (*fireSquare >= burnAge and *fireSquare < rockAge) {				
				...
				*fireSquare=-1;
				...
				while (dnBurning > 0 and doQuench==0) {
					...
					//printf("fire now burning. fireIndex=%d\n", fireIndex);
					while (fireIndex < (nBurning+dnBurning)) {
						// so we'll look at the 'active' part of the fire list and add new fire elements to the end of the active portion.
						// this is element grids[fireIndex]
						//int *atLeft= &grids[0]+fireIndex-1;
						//int *atRight= &grids[0]+fireIndex+1;
						//int *atUp=&grids[0]+fireIndex+(xmax+2);
						//int *atDown=&grids[0]+fireIndex-(ymax+2);
						//
						//int *currentSquare=&grids[0]+fireIndex;
						//
						// check for fire:
