...
//
// 9b: 9a is the standard list-based model. 9b introduces MFI. (i think not related to this code-branch)
//
// 10a: add SHEEP. copy SHEEP related functions and other code from 7k. but i think the 10a version did not work properly. 10a was derived from the SHEEP model (7k)
// for 10b, SHEEP were added to 9a/b (i think). i believe 10b is the working model.
//
...
//	std::string simPramComment("");
	...
	unsigned long totalSheepPoops=0;
	//
...
//			};
		...
		if (rfmatch.nextInt(fmatch) == 1) {
			...
			//if (*fireSquare >= burnAge and *fireSquare < rockAge) {				
				...
				*fireSquare=-1;
				...
				//
				// write to file:
				if (nFires%1000==0 or totalSheepPoops%1000000==0) {
					fstats.open(statFileName.c_str()); 
					fstats <<"# X,Y = " << xmax << ", " << ymax << "\n";
	...
	for (unsigned int i=0; i<(xmax*ymax); i++) {
		if (fcounts[i]!=0 or extinguishedFires[i]!=0 or clusterSizes[i]!=0) {
			// note: we can include immunity if we want; this code-stream was split from ffire9b (i think).
			//fstats << i+1 << "\t" << fcounts[i] << "\t" << extinguishedFires[i] << "\t" << clusterSizes[i] << "\t" << sheepPoops[i] << "\n";
			//fstats << i+1 << "\t" << fcounts[i] << "\t" << extinguishedFires[i] << "\t" << sheepPoops[i] << "\n";
			fstats << i+1 << "\t" << fcounts[i] << "\t" << extinguishedFires[i] << "\t" << clusterSizes[i] << "\t" << sheepPoops[i] << "\n";
			};
		};
	fstats.close();
