...
//
// VERSION 7g: fatty-sparks:
// instead of sheep, use a smart, sheep-walk lightning strike.
//
...
int doffire(unsigned tmax=1000, int doSQL=1, unsigned int fMatch=200, float nRand=1, float nClust=0, int kmax=4, unsigned int sheepSpeed=0) {
...
/*
		...
		//	if (sheepPos<0) sheepPos = sheepPos + (xmax+2)*(ymax+1);
		//	if (sheepPos>=(int(xmax)+2)*(int(ymax)+2)) sheepPos = sheepPos-(int(xmax)+2)*(int(ymax)+1);
			}; // sheep-speed
*/
...
//			};
	...
	// a 1 in fMatch chance (use any value between 0 and fMatch)
		if (rfMatch.nextInt(fMatch) == 1) {
			...
			//
			int dfsqr = (xmax+2)*yFire + xFire;
			fireSquare = grids + dfsqr;
			//printf("match: (%d, %d) :: %d\n", xFire, yFire, *fireSquare);
			//yodapause();
			//
			// 7g: now walk the spark until we run out of walks or we get a fire...
			unsigned isheep=0;
			int dSheepDir = 0;
			while (isheep<sheepSpeed) {
				// printGrid2(&grids[0], i, xmax, ymax, (fireSquare-grids));
				// printf("walk!!! (%d)\n", sheepSpeed);
				if (*fireSquare==0 || *fireSquare==rockAge) {
					dSheepDir = getSheepDir(dfsqr, grids, (xmax+2), sheepRand);
					dfsqr = dfsqr + dSheepDir;
					fireSquare = fireSquare + dSheepDir;
					//printf("dSheepDir: %d\n", dSheepDir);
					//fireSquare = fireSquare + getSheepDir(fireSquare-grids, grids, (xmax+2), sheepRand);
					};
				if (*fireSquare==1) isheep=sheepSpeed;
				isheep=isheep+1;
				};
			// printf("moving on?\n");
