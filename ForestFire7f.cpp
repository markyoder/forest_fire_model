...
//
// VERSION 7f: Space Sheep
// we saw apparent success in 7e. we get a variety of slopes with a funny little bump to some characteristic size fire. sheep and sparks
// would seem to select clusters differently. Pspark -> area(), Psheep-> circumference()-loops(). we observe, however, that our sheep rules
// mix these probabilities by randomly dropping sheep on the grid, in which case they maintain a Psheep->area() component. we introduce two
// variations: a torroidal world for sheep and "space sheep." in the first variation, sheep walk off the top or bottom of the grid to the bottom
// or the top. strict torroids may be skipped on lateral movement for programatic simplicity.
//
//
...
int getSheepDir (unsigned int sheepPos, int *theGrid, int gridX, Rand *thisRand) {
	...
	if (nFoodDirs>0) {
		...
		sheepDir = (thisRand->nextInt(ymax)+1)*(xmax+2) + thisRand->nextInt(xmax) + 1 - sheepPos;	// pick a point on the grid, subtract current pos; go that many...
		};
	//
	return sheepDir;
	};
...
int doffire(unsigned tmax=1000, int doSQL=1, unsigned int fMatch=200, float nRand=1, float nClust=0, int kmax=4, int sheepSpeed=0) {
	// burn toplolgy: do we nave nnn burning
	// 
	// allow for no-fire simulation (aka, sheep only):
	if (fMatch==0) fMatch=tmax;
	//
	std::string simName("ForestFire7f");
	...
	sheepPos = (xmax+2)*(sheepRand.nextInt(ymax)+1) + sheepRand.nextInt(xmax)+1;
...
*/
	...
	while (fcounts[0]<tmax) {
		...
		for (unsigned short isheep = 0; isheep<sheepSpeed; isheep++) {
			switch (*(grids+sheepPos)) {
				case 0: case rockAge:
					// now, with the space-sheep model, we have to limit teleportations or we just get an exponential cutoff.
					// we get one teleportation per turn. regular walk moves are either +1, -1, +/- (xmax+2).
					// another way to do this might be to look at the value of the grid element onto which we step.
					// if we move and step onto an empty square, we're done...
					// move:
					dSheepDir = getSheepDir(sheepPos, grids, (xmax+2), &sheepRand);
					if (dSheepDir==-1 or dSheepDir==1 or dSheepDir==(xmax+2) or dSheepDir==-(xmax+2) or didTeleport==false ) {
						// we are doing a regular walk or our first teleport.
						sheepPos = sheepPos + dSheepDir;
					} else {
						// we are trying to teleport again. exit the loop.
						isheep=sheepSpeed;
						};
					if (dSheepDir!=-1 and dSheepDir!=1 and dSheepDir!=(xmax+2) and dSheepDir!=-(xmax+2)) didTeleport=true;
					break;
				case 1:
					// eat:
					*(grids+sheepPos)=0;
	...
	//			};
			...
			// skip the right/left rules. note we retain rocky boundaries, so sheep-rules persist around the torroid.
			if (sheepPos<0) sheepPos = sheepPos + (xmax+2)*(ymax+2);
			if (sheepPos>=int((xmax+2)*(ymax+2)) ) sheepPos = sheepPos-(int(xmax)+2)*(int(ymax)+2);
			}; // sheep-speed
		...
		//
		// allow kmax descrimination; aka, kmax=2 gives purely dendritic growth...
		//*(grids+gridPos) = 1;
		if (hasNeighbor(grids, gridPos)<=kmax) *(grids+gridPos) = 1;
...
//			};
	...
	if (doSQL==5 ) { // no plots, just a summary -> SQL
		...
		//	printf ("sql bits: %d, %d, %d, %d\n", intKey, tmax, i+1, fcounts[i]);
			...
			myquery.execute(intKey, *maxtime, i+1, fcounts[i]);
			myquery.reset();
			myquery << "update SimPrams set tmax=%0q where simIndex=%1q";
			myquery.parse();
			myquery.execute(*maxtime, intKey);
			//printf("%d,\t%d\n", i+1, fcounts[i]);
			};				
		};
