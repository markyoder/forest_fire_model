...
const unsigned int bigScale = 4;
const unsigned int xmax = bigScale*256;
const unsigned int ymax = bigScale*256;
...
int getClustMaps1(int *bigClustMaps, int X, int Y) {
	int clustGrids[(ymax+2)*(xmax+2)];	// primary map of sub-clusters;
	// initialize.
	for (int i=0; i<(ymax+2)*(xmax+2); i++) {
		clustGrids[i] = 0;
		};
	
	};
// int doffire(unsigned int xmax=128, unsigned int ymax=128, unsigned int fmatch=125 unsigned short burnPropAge=400) {
int doffire(unsigned tmax=1000000, int doSQL=1, unsigned short fMatch=200) {
	// burn toplolgy: do we nave nnn burning
	//
	std::string simName("ForestFire6");
	std::string simPramComment("map4");
	//int fMatch = 250;
	//
	...
	int burnPropAge=1;
	// float aveTreeAge=0;
	int thisClust=0, burningClust=0;
	//
	//int grids[(ymax+2)*(xmax+2)];
//	int clustGrids[(ymax+2)*(xmax+2)];	// map of sub-clusters;
	//short bigScale = 5;
	int *bigClustMaps = new int[(bigScale*ymax+2)*(bigScale*xmax+2)];
	int *grids = new int[(bigScale*ymax+2)*(bigScale*xmax+2)];
	int intKey;

//   int grids[(bigScale*ymax+2)*(bigScale*210+2)]; // ??? can't get 256...
	float aveRho, aveVol;
	//printf("declarations finished.\n");
	//
...
*/
	...
	for (unsigned int iy=192-i2*dpos; iy < 224-i2*dpos; iy++) {
		for (unsigned int ix=0; ix < xmax1; ix++) {
			...
			if ( (iy+1)%4 == 0) thisClust = thisClust + xmax/4;
		};
	...
	for (unsigned int iy=224-i2*dpos; iy < 240-i2*dpos; iy++) {
		for (unsigned int ix=0; ix < xmax1; ix++) {
			...
			if ( (iy+1)%8 == 0) thisClust = thisClust + xmax/8;
		};
	...
	for (unsigned int iy=240-i2*dpos; iy < 256-i2*dpos; iy++) {
		for (unsigned int ix=0; ix < 128; ix++) {
			...
			//if ( (iy+1)%16 == 0) thisClust = thisClust + xmax/16;
		};
	...
	for (unsigned int i=1; i<(ymax1+2); i++) {
		for (unsigned int ii = 1; ii<=bigScale; ii++) {
			for (unsigned int iii=1; iii<(xmax1+2); iii++) {
	...
	int biggestFire = bigScale*bigScale*32;
	//
	// now, the cluster map is established. as we plant trees, we will plant against this grid.
	//printf("cluster map set.\n");
	//plotGridImg (&clustGrids[0], 0, xmax/bigScale, ymax/bigScale, burnPropAge);
	//plotGridImg (bigClustMaps, 0, xmax, ymax, burnPropAge);
	//
	...
	if (doSQL==1 or doSQL==5) {	
		...
		myquery << "insert into ForestFire.SimPrams (SimName, SimSW, xmax, ymax, sparkInterval, tmax, Comment) values (%0q, %1q, %2q, %3q, %4q, %5q, %6q)";
		myquery.parse();
		// note: simIndex(auto-int) and dtTime (default TIMESTAMP) set automatically.
		myquery.execute(simName.c_str(), simName.c_str(), xmax, ymax, fMatch, tmax, simPramComment.c_str());
		//
...
*/
	...
	for (unsigned int i=0; i<=tmax; i++) {
		...
		if (1==1) {
			//thisX = rand()%xmax+1;
			//thisY = rand()%ymax+1;
			thisX = fireRandx.nextInt(xmax);
			thisY = fireRandy.nextInt(ymax);
			// thisSquare = &grids[0] + thisX + 1 + (xmax+2)*(thisY+1);	// point to the current square...
			//thisClust = clustGrids[(thisY+1) * (xmax+2) + thisX + 1];
			//grids[(thisY+1) * (xmax+2) + thisX + 1] = thisClust;		// set "forest" grid value to the cluster values. fires will propagate within a cluster.
			thisClust = *(clustGrids + (thisY+1) * (xmax+2) + thisX + 1);
			*(grids + (thisY+1) * (xmax+2) + thisX + 1) = thisClust;
			};
		...
		if (rfMatch.nextInt(fMatch) == 1) {
			...
			//if (yFire>188) printf("bigx!\n");
			//
			//fireSquare = &grids[0] + (xmax+2)*yFire + xFire;
				...
				while (dnBurning > 0) {
					...
					for (int iy = (yFireMin); iy <= (yFireMax); iy++) {
						for (int ix = (xFireMin); ix <= (xFireMax); ix++) {
							...
							//
							// no immunity:
							if (cStat == burningClust and cStat < rockAge 
								and (lStat==-burningClust or rStat==-burningClust or uStat==-burningClust or dStat==-burningClust))
								{
//
...
//					myquery.execute(intKey, i, xFire, yFire, aveTreeAge, nBurning);
				...
				if (doSQL==3) {
					...
					if (nBurning>=1) plotGridSimple (grids, i, xmax, ymax);
					};
				if (doSQL==4) {
					if (nBurning>=1) plotGridImg (grids, i, xmax, ymax, burnPropAge);
					};
				//
...
//	if (argv[6]) burnAge=yodacode::StrToInt(argv[6]);
